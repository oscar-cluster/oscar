...
package OSCAR::OCA::PM_Abstract::urpmi;

use strict;

# check for:
# - /usr/bin/urpmi			=> package management
# - /usr/bin/urpmi.addmedia		=> repo management
# - /usr/bin/createrepo			=> repo creation
BEGIN {
	die "urpmi work in progress"; # BUG remove when done.
	die "urpmi unavailable on this system" if(! -x "/usr/bin/urpmi"); # Prevent module loading if urpmi not available.
}
...
sub pm_command {
	...
	if ($command eq "install") {
		...
		return system("urpmi $options $pkgs");
	} elsif ($command eq "update") {
		$pkgs = join (' ', @args) if ( scalar @args == 0);
		return system("urpmi --update $options $pkgs");
	} elsif ($command eq "reinstall") {
		return -1 if ( scalar @args == 0); # no pkg names provided: error
		$pkgs = join (' ', @args);
		return system("urpmi --force $options $pkgs");
		...
		$pkgs = join (' ', @args);
		return system("urpmf --provides $options '$pkgs'");
