...
package OSCAR::OCA::PM_Abstract::yum;

use strict;

# check for:
# - /usr/bin/yum			=> package management
# - /usr/bin/yum-config-manager		=> repo management
# - /usr/bin/createrepo			=> repo creation
BEGIN {
	die "yum unavailable on this system" if(! -x "/usr/bin/yum"); # Prevent module loading if yum not available.
}
...
sub pm_command {
	...
	if ($command eq "install") {
		...
		return -1 if ( scalar @args == 0); # no pkg names provided: error
		$pkgs = join (' ', @args);
		return system("yum $options info $pkgs");
