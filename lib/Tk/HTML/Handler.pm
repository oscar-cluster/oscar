...
use vars qw($VERSION);
$VERSION = '3.002'; # $Id: Handler.pm,v 1.1 2002/10/28 22:43:56 tfleury Exp $
...
my %FontTag = ('CITE' => 'I', 'STRONG' => 'B', 'EM' => 'I', 
                      'TT' => 'KBD', 'SAMP' => 'CODE');
...
sub OptionText
{
 ...
 #while (($package,$filename,$line,$subroutine,$hasargs,$wantarray,$evaltext,
 #   $is_require,$hints,$bitmask) = caller($i++))
 #  {
 #    print "Level = $i, pack = $package, file = $filename, line = $line, sub = $subroutine\n";
 #  }
 ...
 if (defined $elem)
  {
   ...
   if ($elem->attr('selected'))
     {
       $singleselecttext = $text;
       $singleselectval = $val;
     }
   $mb->setOption($singleselecttext,$singleselectval) if ($singleselecttext);
  }
 else
  {
 ...
 if (defined $elem)
  {
   ...
   $lb->selection('set',$index) if ($elem->attr('selected'));
  }
 else
  {
   confess "$text outside option";
 ...
 if ($f) 
  {
   ...
   $h->{'option'} = [];
   if ($elem->attr('multiple') || ($elem->attr('size') > 1)) 
    {
     my $size = $elem->attr('size');
     $size = 10 unless ($size);
     my $e = $w->Scrolled('Listbox',-height => $size,
                          -scrollbars => 'e', -exportselection => '0');
...
sub textarea 
{
  ...
  if ($f) 
   {
    my $rows = $elem->attr('rows') || 10;
    my $cols = $elem->attr('cols') || 20;
    my $wrap = 'none';
    $wrap = 'word' if ((defined $elem->attr('wrap')) &&
      (($elem->attr('wrap') =~ /hard/i) || ($elem->attr('wrap') =~ /soft/i)));
    my $form = $h->CurrentForm;
    my $w = $h->Widget;
    $elem->{'NAME'} = '__inconnu__' if ! defined $elem->{'NAME'};
    my $t = $w->Scrolled('Text',-wrap => $wrap,  -relief => 'sunken', -scrollbars => 'se',
