% -*- latex -*-
%
% $Id: design.tex,v 1.25 2002/10/24 20:44:55 bwbarrett Exp $
%
% $COPYRIGHT$
%
...
\item \xmltag{rpmlist}: A top-level tag supplying lists of RPMs to be
  ...
  listed, includes a list of RPM names.  Primitive conflict resolution
  is run on this list, checking for such things as two versions of the
  same RPM, etc.  However, the majority of burden for RPM conflict
  resolution is up to the package author -- don't include RPMs that
  will be problematic.\footnote{ Perhaps someday we can relax this
  ...
  one scoping tag containing at least one valid RPM name.
