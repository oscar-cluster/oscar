% -*- latex -*-
%
% $Id: design.tex,v 1.20 2002/10/08 07:19:04 jsquyres Exp $
%
% $COPYRIGHT$
%
...
are present, then all RPMs will be installed on all nodes.
...
\begin{itemize}
\item \xmltag{name}: This is a string tag that is the name
  of the package.  It should be the same as the package's directory
  name in the \file{packages/} directory.
...
\item \xmltag{uri}: A string presenting the URI of where either the
  OSCAR package or the native package itself may be found.
  
  Optional/Singular.  If not supplied, the value
  ``\xmlval{$<$none$>$}'' is used.
  
\item \xmltag{packager}: A string the name and/or the e-mail address
  of the person who created/maintains the OSCAR package.  E-mail
  addresses should be presented in common RFC 2822 form -- surrounded
  by ``$<>$''.  For example, ``{\tt Joe Shmoe $<$jshmoe@foo.com$>$}''.
...
\item \xmltag{provides}: A top-level tag effectively indicating
  ...
  Optional/Singular.  If not supplied, no synonyms for this package
...
\item \xmltag{requires}: A top-level tag effectively indicating
  ...
  \item \xmltag{type}: A string indicating whether the dependency is
    ...
    \item \xmlval{oscar-package}: the dependency is another OSCAR
      package.
    \end{itemize}
  ...
  \item \xmltag{name}: A string indicating the name of the RPM or
    OSCAR package that this package depends on.

    Mandatory/Single.
    
  \item \xmltag{distro\_name}: A string indicating which Linux
    distribution the dependency exists on.  Possible values are:

    \begin{itemize}
    \item \xmlval{RedHat}: indicates that the dependency exists on
      RedHat Linux distributions.
    \item \xmlval{Mandrake}: indicates that the dependency exists on
      Mandrake Linux distributions.
    \item \xmlval{all}: indicates that the dependency exists on all
      Linux distributions.
    \end{itemize}
    
    Optional/Singular.  If not specified, ``\xmlval{all}'' is used.
    
  \item \xmltag{distro\_version}: A number indicating the version 
    of Linux distribution the dependency exists on.
...
\item \xmltag{conflicts}: A top-level tag effectively indicating
  ...
  \begin{itemize}
  \item \xmltag{type}: A string indicating whether the conflict is an
    RPM or another OSCAR package.  Possible values are:

    \begin{itemize}
    \item \xmlval{rpm}: the conflict is an RPM.
    \item \xmlval{oscar-package}: the conflict is another OSCAR
      package.
    \end{itemize}
  ...
  \item \xmltag{name}: A string indicating the name of the RPM or
    OSCAR package that this package conflicts with.

    Mandatory/Single.
    
  \item \xmltag{distro\_name}: A string indicating which Linux
    distribution the conflicts exists on.  Possible values are:

    \begin{itemize}
    \item \xmlval{RedHat}: indicates that the conflict exists on
      RedHat Linux distributions.
    \item \xmlval{Mandrake}: indicates that the conflict exists on
      Mandrake Linux distributions.
    \item \xmlval{all}: indicates that the conflict exists on all
      Linux distributions.
    \end{itemize}
  ...
  \item \xmltag{distro\_version}: A number indicating the version of
    Linux distribution the conflict exists on.
...
\begin{discuss}
  If we want to replace/supplement the \file{server.rpmlist} and
  \file{client.rpmlist} files, additional XML tags should be
