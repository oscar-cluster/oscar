...
%
% $Id: release-notes.tex,v 1.25 2002/11/16 19:24:16 jsquyres Exp $
%
...
\item Although OSCAR can be installed on pre-existing server nodes, it
  ...
  there may be conflicts in RPM requirements.  It is recomended to
...
\begin{itemize}
\item This is the second release of OSCAR to support this distribution.
  OSCAR \oscarversion\- has been tested and successfully installed on
  this distribution (see Table~\ref{tab:oscar-distro-support});
  
\item Before any OSCAR installation, you should install the following
  perl packages
\begin{verbatim}
  # urpmi perl-XML-Simple perl-XML-Parser
\end{verbatim}
  
\item There is a problem with the TCL libraries -- the RPM was
  compiled on Mandrake.  In order to avoid an error, make the
  following symlink before running the OSCAR installer:
