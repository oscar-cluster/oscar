...
%
% $Id: release-notes.tex,v 1.42 2003/11/26 03:36:11 bernardli Exp $
%
...
\begchange
\item If you are SSHing into the OSCAR headnode and try to run cexec, you might
  see a benign message in the cexec output:
...
\end{verbatim}
  ...
  This is actually just a warning message however if you are having problems
  using the 
  \\ \button{Install/Uninstall Package} feature, you may want to
  disable X forwarding.  You can do this by setting the \cmd{C3\_RSH} environment
  variable to \cmd{ssh -x}.  You can do this by adding the following line to the
  \cmd{root} user's \cmd{.bash\_profile} file:

\begin{verbatim}
  export C3_RSH=ssh -x
\end{verbatim}

  All subsequent shells will have X forwarding disabled.

\item During the \button{Install/Uninstall Package} step, if you hit the cancel
  button or close down the window, the post* (post installation) scripts will still run.  
  This is 
  normal and should not really
  affect anything (i.e. if you did not pick any packages to install/uninstall, they
  will not be).
