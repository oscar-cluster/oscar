...
\begchange
be permanently installed on the system in \file{/opt/oscar} (default
\endchange
...
<<<<<<< .mine
  ...
  For each CD, locate the directory that contains the RPMs.  In Fedora Core
...
OPD also will make use of the ``{\tt http_proxy}'' environment
...
\begchange If your network switch supports multicasting, there is a
new feature in OSCAR 3.0 which uses multicast to push files to the
clients as part of the Network Boot option.  To enable this feature
simply click on the \button{Enable Multicasting} checkbox.
...
using rsync, simply go back to the \button{Setup Networking} menu, make
sure that the \button{Enable Multicasting} checkbox is disabled (should be
disabled by default) and then click on \button{Configure DHCP Server} -
...
<<<<<<< .mine
The package installation system is designed to help simplify adding
and removing OSCAR packages after the initial system installaion.
The system is fairly straightforward for the user as packages are
selected for the appropriate action from the wizard.
=======
