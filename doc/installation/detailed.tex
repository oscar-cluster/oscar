% -*- latex -*-
%
% $Id: detailed.tex,v 1.46 2002/09/14 20:38:14 jsquyres Exp $
%
% $COPYRIGHT$
%
...
For example, if you want OSCAR to install the Ganglia package,
download the Ganglia OSCAR tarball from the main OSCAR SourceForge
site.  Expand it into the \file{packages/} directory in the OSCAR tree
(assuming that \file{ganglia-oscar-\oscarversion.tar.gz} is in
\user{root}'s home directory):
...
\file{oscar-\oscarversion/packages/ganglia} with all the files
...
\file{README} file and \file{doc/ganglia-user.pdf} which should both
