% -*- latex -*-
%
% $Id: detailed.tex,v 1.28 2002/07/15 17:02:27 loann09 Exp $
%
% $COPYRIGHT$
%
...
Begin the installation process by pressing the Step 1 button of the wizard entitled \button{Prepare OSCAR Server for Install}. Here you will select the default MPI implementation for use on the cluster. This step also installs and starts other necessary software and devices.
\endchange
...
the version, architecture, or extension of the rpm filename. For example, \file{bash-2.05-8.i386.rpm} is simply \file{bash}. \endchange
...
by System Configurator from the contents of the \file{systemconfigurator.conf} file. See the \file{systemconfig.conf} man page for full details on the
...
cases. At a minimum, you will need to enter a value in the \field{Number of Hosts}
...
To start the collection, press the \button{Collect MAC Address} button and then
...
%%=======
%%>>>>>>> 1.27
Press the Step 6 button of the wizard entitled \button{Test
 Cluster Setup}. This will open a window that will prompt you for the number clients and number of processors per client. The systems \user{root} services are checked and then a set of user level tests are run. A sample dialog is shown in Figure~\ref{fig:detailed-setup-test}. If any of the test fail, then there is a problem with your installation.
\endchange 
...
Like before, you must substitute the device name (e.g., \file{eth0}) for your server's internal ethernet adapter in the above command. See Section~\ref{det:installcluster}. Once the OSCAR wizard appears you are ready to add or delete clients. Note that these steps will reuse the existing images made with the initial install, however it will extend or contract the your defined clients.

\subsubsection{Adding OSCAR clients}
Press the button of the wizard entitled \button{Add OSCAR Clients}. A sample dialog is shown in Figure~\ref{fig:detailed-add-node}. These steps should seem familiar. They are same as steps 3, 4 and 5 of the initial install. Refer to Sections~\ref{det:defclients}, \ref{det:setupnetwork}, and \ref{det:completeinstall}.
...
Press the button of the wizard entitled \button{Delete OSCAR Clients}. A sample dialog is shown in Figure~\ref{fig:detailed-delete-node}. Select the node you wish to delete and simply press the button \button{Delete clients} then press \button{Close}.
