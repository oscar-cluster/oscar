...
\begchange
\begin{itemize}
\item The \button{Build Autoinstall CD} button will build an iso image
  for a bootable CD and gives a simple example for using the cdrecord
  utility to burn the CD. This option is useful for client nodes that do
  not support PXE booting.  In a terminal, execute the command
  {\tt cdrecord -scanbus} to list the drives cdrecord is aware of
  and their dev number.  Use this trio of numbers in place of
  {\tt dev=1,0,0} when you execute the command
  {\tt cdrecord -v speed=2 dev=1,0,0 /tmp/oscar\_bootcd.iso}.
...
\subsection{Monitor Cluster Deployment}
\label{det:si_monitortk}

\optional

During the client installation phase it is possible to click on the 
\button{Monitor Cluster Deployment} button to bring up the SystemImager
monitor GUI.  This window will display real-time status of nodes
as they request images from the server machine and track their progress
as they install.

  \begin{figure}[htbp]
    \begin{center}
      \includegraphics[scale=\imgscale]{figs/si_monitortk}
      \caption{Monitor Cluster Deployment}
      \label{fig:detailed-si-monitortk}
    \end{center}
  \end{figure}
...
You can use the \button{Monitor Cluster Deployment} functionality to
monitor the progress. Depending
