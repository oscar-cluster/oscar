...
%
% $Id: detailed.tex,v 1.54 2003/01/16 23:23:37 jsquyres Exp $
%
% $COPYRIGHT$
%
...
\detailed{
  ...
  \file{/tftpboot/rpm} directory with a command such as:
...
\quick{The OPD can be launched by running the command \cmd{opd} from
  the top-level OSCAR \file{scripts} directory.}
...
Setting the \env{OSCAR\_\-HOME} environment variable before invoking
...
OSCAR installer.  If you don't set \env{OSCAR\_\-HOME}, OPD will place
downloaded distribution packages in \file{\$HOME/.opd/downloads}, and
...
\detailed{
  ...
  actually download them.  If you set the \env{OSCAR\_\-HOME}
  environment variable before running OPD, OPD will unpack the
  downloaded packages in the correct directory in the OSCAR tree.
...
\detailed{
  ...
  on the \button{Save and Exit} button to save your selections and
  return to the main OSCAR window.  Click on the \button{Exit without
    Saving} button to discard your changes and use the list of
  selections that was initial displayed when you initially opened the
  window.
...
\detailed{
  ...
  are appropriate for that package, and then click on the \button{Save
    and Exit} button to save your selections, or \button{Exit without
    Saving} to cancel all of your selections and leave the original
  settings.
...
\begin{itemize}
\item Ensure that {\tt PermitRootLogin} is set to {\tt yes}.  After
  the OSCAR installation, you may set this back to {\tt no} (if you
  want), but it needs to be {\tt yes} during the install because the
  config file is copied to the client nodes, and \user{root} {\em
    must} be able to login to the client nodes remotely.

\item By the same token, ensure that TCP wrappers settings are not
  ``too tight''.  The \file{hosts.allow} and \file{hosts.deny} files
  should allow all traffic from the entire private subnet.
...
boot your client nodes.  \detailed{For instructions on doing so, see
  Appendix~\ref{app:net-boot-client-nodes}. A sample dialog is shown
