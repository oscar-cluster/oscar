...
%
% $Id: detailed.tex,v 1.49 2002/11/17 04:02:39 jsquyres Exp $
%
% $COPYRIGHT$
%
...
When installing Linux, it is not necessary to perform a ``custom''
...
download them.  If you set the \env{OSCAR\_\-HOME} environment
...
the \button{Save and Exit} button to save your selections and return
to the main OSCAR window.  Click on the \button{Exit without Saving}
...
Clicking on any of the packages' \button{Config} button will bring up
a panel for configuring that package.  Select whatever options are
appropriate for that package, and then click on the \button{Save and
  Exit} button to save your selections, or \button{Exit without
...
before building the client image).  See the \cmd{kernel\_\-picker}
...
\button{Close} button in the build clients dialogue and continue with
the next step.
...
checked and then a set of \user{root} and user level tests are run. A
