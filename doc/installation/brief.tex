% -*- latex -*-
%
% $Id: brief.tex,v 1.23 2002/09/11 07:17:59 jsquyres Exp $
%
% $COPYRIGHT$
%
...
to read the release \docswitch{notes in
  Section~\ref{sec:release-notes}.}{notes in the full installation
  guide.}
...
\item Ensure to assign a hostname that is not ``{\tt localhost}'' or
  ...
  \item If the server node is to be connected to a public network, the
    ...
    \rpmname{pfilter} that may be used for this).
...
\item Download any optional third party OSCAR packages and unpack
  ...
  it into the \file{packages/} directory in the OSCAR tree (assuming
  that \file{ganglia-oscar-\oscarversion.tar.gz} is in \user{root}'s
  home directory):
...
\item Get all the relevant updates for your Linux distribution and
  ...
  updated RPM files to \file{/tftpboot/rpm} (along with the RPMs from
  the installation CDs).
  
  \sbsdetail{det:distro-updates}
  
\item Configure the second (private) cluster network adapter using a
  command such as \cmd{netcfg}, \cmd{neat}, or your favorite editor.
...
\end{verbatim}
  ...
  In the above command, substitute the device name (e.g., \file{eth1})
...
\item Press the \button{Define OSCAR Clients} button on the OSCAR
  ...
  Press the \button{Addclient} button to define the clients. Once that
