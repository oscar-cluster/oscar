% -*- latex -*-
%
% $Id: brief.tex,v 1.14 2002/07/15 17:02:27 loann09 Exp $
%
% $COPYRIGHT$
%
...
installation instructions \docswitch{in Section~\ref{det:top}
  (page~\pageref{det:top}).  Throughout the quick install
...
\item After the installation is complete, log on as \user{root} to the server
  machine.
...
\item Configure the second (private) cluster network adapter using the
  linux \cmd{/usr/sbin/netcfg} command or your favorite editor. Set
...
\cmd{install\_cluster} successfully completes some configuration, it
...
\item Press the \button{Build OSCAR Client Image} button.  This pops
  ...
  \file{sample.disk.ide} \& \file{sample.disk.scsi} in the
...
\item Press the \button{Define OSCAR Clients} button on the OSCAR
  ...
  the \button{Addclient} button to define the clients. Once that is
  complete a dialog will pop up.  If it is successful, press the
  \button{Close} button on the pop up window and then the
  \button{Close} button on the ``Define OSCAR Clients'' window.
...
\item Press the \button{Setup Networking} button on the OSCAR Wizard.
  ...
  diskette in the floppy drive and press the \button{Build Autoinstall
    Floppy} button. You may create several diskettes if you like.
...
\begchange   Press the \button{Test Cluster Setup} button. A window will
