% -*- latex -*-
%
% $Id: brief.tex,v 1.20 2002/07/20 17:20:29 jsquyres Exp $
%
% $COPYRIGHT$
%
...
to read the release \docswitch{notes in
  Section~\ref{sec:release-notes}.}{notes in the full installation
    guide.}  There are important references for RedHat 7.1 and 7.2
...
\begchange
  ...
  \item Ensure that the \rpmname{ipchains} firewall software package
    is not installed.
  ...
  \item If the server node is to be connected to a public network, the
    ...
    \rpmname{pfilter} that may be used for this, but does not turn it
    on by default).
