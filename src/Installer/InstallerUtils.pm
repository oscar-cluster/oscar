...
correspond to the <command>, <test>, and <error> tags
in the GUI.xml file, which act as prerequisites for a Task.  All of the
commands corresponding tests must return true for the Task to be able to
run.  So, you set the C<$activeOdaCommand> and C<$activeTestCode> variables
and then call C<runActiveOdaTest()> for each oda prereq.  If the prereq
fails, then the C<$activeErrorCode> is executed and the resulting string is
output to the "Details" pane of an error dialog box.  If the
C<$activeErrorCode> is empty, it defaults to the output of
C<getDefaultErrorString()>.
...
This subroutine returns the full path of the script which was envoked.
...
Run the current oda command and associated test code and save the output
variables in some package-scope variables.  The following variables are set
by this subroutine:
  ...
  $testCodeSuccess   - if $odaCommandSuccess is 1, then run the test code
                       and save the return result in this variable
...
along with an error string indicating the cause of the failure.  The
developer has the option of specifying code to return an error string for
each oda command/test.  If no such error string is specified, then a default
error string is returned by C<getDefaultErrorString()>.  
...
developer might write something like this:
    ...
    <error>use InstallerUtils; 
           return "Problem with the ODA database:\n" . getDefaultErrorString();
    </error>
