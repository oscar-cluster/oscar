...
\subsection{Ranges On the Command Line}
...
The second case is a range on the default cluster. This is in a form
of <:range>. An example cexec would be as follows:

\cmd{cexec :1-4,6 ls -l}

This would execute ls on nodes 1,2,3,4,6 of the default cluster. The
third method is specifying a specific cluster. This takes the form
of <cluster\_name:>. And example cexec would be as follows:

\cmd{cexec test: ls -l}

This would execute ls on every node in cluster test. The fourth and
...
This would execute ls on nodes 2,3,4,10 on cluster test. These four
methods can be mixed on a single command line. for example

\cmd{cexec :0-4 htorc1: htorc2:1-5 ls -l}

is a valid command. it would execute ls on nodes 0,1,2,3,4 of the
